...
export function TransferReceivePage() {
  ...
  const puedeRecibir = tarima?.estado === "EN_TRANSITO";
      ...
      {!tarima ? (
        ...
        <>
          <div className="card">
            ...
            <div className="result-row"><span className="result-label">Estado</span><span className="status-badge status-transito">{tarima.estado}</span></div>
          </div>

          {puedeRecibir ? (
            <button className="btn btn-primary" onClick={handleReceive} disabled={receiving}>
              {receiving ? "Recibiendo..." : isOnline() ? "📥 Recibir en esta bodega" : "📥 Guardar para después"}
            </button>
          ) : (
            <div className="card" style={{ textAlign: "center", background: "#fff3e0" }}>
              <p style={{ color: "#e65100", fontSize: 14 }}>
                La tarima no está en tránsito o ya fue recibida
              </p>
            </div>
          )}
